...
export async function GET(req: NextRequest) {
  try {
    ...
    // Column indices: H = 7 (Loan Cycle)
    const colH = 7; // Loan Cycle
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // Get loan cycle from COL H
      const cycleValue = row[colH];
    ...
    console.log(`Processed loan cycles from sheet "${sheetName}"`);

    return NextResponse.json(cycleData);
  } catch (err) {
    console.error("Error parsing Excel file for loan cycles:", err);
    return NextResponse.json(
