...
export async function GET(req: NextRequest) {
  try {
    ...
    const row17 = 16; // Row 17 (0-based index)
    const row18 = 17; // Row 18 (0-based index)

    // Extract values from C17 and C18
    const repayment = data[row17]?.[colC];
    const arrears = data[row18]?.[colC];
